...
router.get("/signup", auth.isAuthenticated, auth.isClienteADM, function(
  req,
  res,
  next
) {
  res.render("client/managerRegistration", {
    title: "Cadastro de Gestores",
    layout: "layoutdashboardclientadm"
  });
});

router.get("/list", auth.isAuthenticated, auth.isClienteADM, (req, res) => {
  Manager.getByClient(req.session._id)
    .then(managers => {
      res.render("client/managerList", {
...
router.get("/edit/:id", auth.isAuthenticated, auth.isClienteADM, (req, res) => {
  Manager.getById(req.params.id).then(manager => {
    res.render("client/managerRegistrationedit", {
...
router.post("/signup", auth.isAuthenticated, auth.isClienteADM, function(
  req,
  res,
  next
) {
    ...
    .then(userF => {
      ...
      Manager.create(ativa)
        ...
        .catch(error => {
          req.flash(
            "danger",
            "O código do Gestor inserido já está sendo utilizado"
          );
          console.log(error);
          res.redirect("/manager/signup");
        });
...
router.post("/:id", auth.isAuthenticated, auth.isClienteADM, (req, res) => {
  ...
  Manager.update(req.params.id, manager)
    .then(() => {
      res.redirect("/manager/list");
    })
    .catch(error => {
